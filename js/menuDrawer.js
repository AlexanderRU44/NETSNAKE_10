...
export class MenuDrawer {
    ...
    drawPixelMenu(title) {
        ...
        if (this.game.currentScreen === "MAIN") {
            ...
            let options = this.game.gameOver ? 
                [t.newGame, t.modesMenu, t.modeInfoMenu, t.settings, t.records, t.tasksMenu, t.achievementsMenu, t.aboutMenu, t.multiplayerMenu] :
                [t.continue, t.newGame, t.modesMenu, t.modeInfoMenu, t.settings, t.records, t.tasksMenu, t.achievementsMenu, t.aboutMenu, t.multiplayerMenu];
        ...
        } else if (this.game.currentScreen === "MULTIPLAYER_MENU") {
            this.drawMultiplayerMenu();
        } else if (this.game.currentScreen === "WAITING_MP") {
            this.drawWaitingScreen();
        } else if (this.game.currentScreen === "MULTIPLAYER_RESULT") {
            this.drawMultiplayerResult();
        }
    }
    ...
    drawMultiplayerMenu() {
        ...
        const options = [t.createRoom, t.joinRoom, t.back];
        options.forEach((opt, idx) => {
            let y = 135 + (idx * 40);
            if (this.game.multiplayerMenuSelection === idx) ctx.fillText(">", 45, y);
            ctx.fillText(opt, 75, y);
        });
    }
    ...
    drawWaitingScreen() {
        ...
        ctx.font = "10px 'Press Start 2P'";
        ctx.fillStyle = this.game.opponentReady ? "#7ed321" : "#ff5c5c";
        ctx.fillText(this.game.opponentReady ? t.opponentReady : t.waitingForOpponent, 200, 140);
        
        if (!this.game.opponentReady) {
            ctx.fillStyle = "#ffffff";
            ctx.font = "8px 'Press Start 2P'";
            ctx.fillText(t.pressOkWhenReady, 200, 200);
        }
        
        if (this.game.multiplayerManager?.roomId) {
            ctx.font = "7px 'Press Start 2P'";
            ctx.fillStyle = "#8b949e";
            ctx.fillText(`${t.roomCode}: ${this.game.multiplayerManager.roomId}`, 200, 260);
            ctx.fillText(t.shareCode, 200, 285);
        }
    ...
    drawMultiplayerResult() {
        ...
        const resultText = this.game.multiplayerResult === 'win' ? t.victory : t.defeat;
        ctx.fillStyle = this.game.multiplayerResult === 'win' ? "#7ed321" : "#ff5c5c";
        ctx.fillText(resultText, 200, 100);
        
        ctx.font = "11px 'Press Start 2P'";
        ctx.fillStyle = "#ffffff";
        ctx.fillText(`${t.score}: ${this.game.score} : ${this.game.opponentScore}`, 200, 180);
        
        ctx.font = "10px 'Press Start 2P'";
        ctx.fillStyle = "#58a6ff";
        ctx.fillText(t.okBackToMenu, 200, 300);
    }
