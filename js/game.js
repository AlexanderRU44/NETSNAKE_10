...
export class Game {
    constructor(canvas, scoreElement, hiScoreElement, hudElement, timerContainer, timerBar, nameOverlay, nameInput, overlayLabel, btnSaveName, joinOverlay, roomCodeInput, btnJoinRoom, btnCancelJoin) {
        this.canvas = canvas;
        ...
        this.joinOverlay = joinOverlay;
        this.roomCodeInput = roomCodeInput;
        this.btnJoinRoom = btnJoinRoom;
        this.btnCancelJoin = btnCancelJoin;
        ...
        // Настройка обработчиков мультиплеера
        this.setupMultiplayerHandlers();
    }

    setupMultiplayerHandlers() {
        if (this.btnJoinRoom) {
            this.btnJoinRoom.onclick = () => {
                const roomId = this.roomCodeInput.value.trim();
                if (roomId) {
                    this.joinMultiplayer(roomId);
                    this.joinOverlay.style.display = "none";
                }
            };
        }
        if (this.btnCancelJoin) {
            this.btnCancelJoin.onclick = () => {
                this.joinOverlay.style.display = "none";
    ...
    updateHUD() {
        ...
        let turnTag = "";
        if (this.isMultiplayer && !this.isPaused && !this.gameOver) {
            turnTag = this.canMove ? ` | ${t.yourTurn}` : ` | ${t.opponentTurn}`;
        }

        this.scoreElement.innerText = `${t.score}:${this.score}${aiTag}${modeTag}${shieldTag}${turnTag}`;
    ...
    endGame(victory = false, win = false) {
        ...
        if (this.isMultiplayer && this.multiplayer) {
            const winner = win ? this.multiplayer.playerId : (this.multiplayer.playerId === 0 ? 1 : 0);
            this.multiplayer.sendGameOver(winner);
            this.victoryFlag = win;
        } else if (victory && this.currentModeIdx === 5) {
        ...
        this.updateHUD();
        if (!this.isMultiplayer) this.sendScoreToFirebase(this.score);
        
        // Выход из мультиплеерной комнаты при окончании игры
        if (this.isMultiplayer) {
    ...
    reset() {
        ...
        // Сброс мультиплеерных переменных при не-мультиплеере
        if (!this.isMultiplayer) {
            this.opponentSnake = [];
            this.opponentScore = 0;
        }
    ...
    handleMenuPress() {
        initAudio();
        if (this.currentScreen !== "EDIT_NAME" && this.currentScreen !== "INTRO" && this.currentScreen !== "JOIN_MULTIPLAYER") {
            if (!this.isPaused) {
                this.isPaused = true;
            }
            this.currentScreen = "MAIN";
        ...
        } else if (this.currentScreen === "JOIN_MULTIPLAYER") {
            this.joinOverlay.style.display = "none";
            this.currentScreen = "MAIN";
        }
    }
    ...
    handleBackPress() {
        ...
        if (this.currentScreen !== "EDIT_NAME" && this.currentScreen !== "INTRO") {
            if (["SETTINGS", "LEADERBOARD", "TASKS", "ACHIEVEMENTS", "MODES", "ABOUT", "MODE_INFO", "MULTIPLAYER", "WAITING"].includes(this.currentScreen)) {
                this.currentScreen = "MAIN";
                if (this.currentScreen === "WAITING" && this.isMultiplayer) {
                    this.leaveMultiplayer();
                }
            } else if (this.currentScreen === "JOIN_MULTIPLAYER") {
                this.joinOverlay.style.display = "none";
    ...
    handleInput(act) {
        ...
        initAudio();
        if (!this.isPaused) {
            if (this.isMultiplayer && !this.canMove) return;
            ...
            if (this.currentScreen === "MAIN") {
            ...
            else if (this.currentScreen === "MULTIPLAYER") {
                let max = 1;
                ...
                if (act === "CENTER" && this.multiplayer?.isHost) {
                    this.setReady();
                }
            }
    ...
    gameLoop() {
        if (this.currentScreen === "INTRO" || this.currentScreen === "EDIT_NAME" || this.currentScreen === "JOIN_MULTIPLAYER") {
        ...
        if (this.isPaused) {
            ...
            if (this.isMultiplayer && this.opponentSnake.length) {
                this.renderer.drawOpponentSnake(this.opponentSnake);
            } else if (this.aiOpponent && this.currentModeIdx === 5) {
            ...
            else if (this.currentScreen === "MULTIPLAYER") this.menuDrawer.drawMultiplayerMenu();
            else if (this.currentScreen === "WAITING") this.menuDrawer.drawWaitingScreen();
            return;
        }
        ...
        // Отправляем ход в мультиплеере после обработки
        if (this.isMultiplayer && !this.isPaused && !this.gameOver && this.multiplayer && this.canMove) {
